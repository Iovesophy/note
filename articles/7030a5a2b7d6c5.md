...
published: true
---
# 超細分化 CloudFormation RootStackTemplate
...
特に今回のように超細分化してCloudFormationTemplateを記述するとなるとすぐに見るべきファイルを見失いがちです。
...
AWSTemplateFormatVersion: 2010-09-09
```
この宣言に関してはCloundFormationを記述する上での例えて言えば、Shell等のShebang的な役割を持っていて、どのAWSCFnのバージョンで実行させるかと言うことを宣言できます。
...
またこのフィールドはAWSのどのリソースについての記述なのかと言うことを宣言するために次の記述が必要になります。
...
CidrBlock: 10.0.0.0/16
```
これはクラスレスインタドメインルーティングブロックを定義します、今回はクラスA、
...
他のCloudFormationテンプレートから出力した値を使用しますという宣言をすることができます。
これは、例えばあるリソースを、別のもしくは同名のエクスポート名に置き換えると、別のテンプレートからこのリソースのIDを取得できます。

具体的の他のCloudFormationテンプレートからどのように扱えば良いのか、
ポイントは二つです。
...
# 最後に
今回作成したテンプレートには一つAWSのベストプラクティス的には非推奨の構成があります。
